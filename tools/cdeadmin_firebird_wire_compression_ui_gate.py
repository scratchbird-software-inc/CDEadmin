#!/usr/bin/env python3
"""Linux compression controls, persistence and actual native reconnection.

Windows/macOS teams repeat with platform native libraries and browsers.
Negotiated compression is asserted separately by the native matrix, not
inferred from a successful browser Save or Services authentication.
"""
...
def profile_checks(context):
    ...
    for policy, compression in (('Required', True), ('Required', False),
        ...
        wait.until(form)
        checkbox = visible_named_control(form(driver),
                                         'Enable wire compression')
        description = driver.execute_script(
            'return (arguments[0].getAttribute("aria-describedby") || "")'
            '.split(/\\s+/).map(id => document.getElementById(id)?.textContent'
            ' || "").join(" ")', checkbox)
        assert ('Client preference for native network compression' in
                description)
        fill_fields(wait, [f'Wire encryption policy={policy}',
                           'Enable wire compression=' +
                           str(compression).lower()], control_root=form)
        ...
        def persisted(_):
            ...
            return (values.get('wire_crypt') == policy and
                    values.get('wire_compression') is compression)

        wait.until(persisted)
        pages = screenshot_form_pages(
            driver, context.evidence / f'compression-{len(observations) + 1}',
            selector=selector)
        ...
        observations.append({'policy': policy, 'compression': compression,
                             'status': response['status'], 'pages': pages})
    return {'compression_profiles': observations, 'keyboard_save': True,
            'accessible_compression_help': True}
