#!/usr/bin/env python3
"""Fixed-width OCTETS array native fidelity and transaction finality."""
import argparse
import json
...
def verify(connection, client, route, password, result, *, binary_only=False):
    ...
    for width in (8, 256):
        table, view = f'OCTA_{width}', f'OCTAV_{width}'
        sql(connection, f'CREATE TABLE {table}(ID INTEGER PRIMARY KEY, '
            f'A CHAR({width})[-1:0,2:3] CHARACTER SET OCTETS)')
        connection.commit()
        ...
        for kind, name in (('table', table), ('view', view)):
            for action in ('commit', 'rollback'):
                ...
                try:
                    original = [[bytes(range(width)), b'\0\xff'],
...
def main():
    ...
    result['complete'] = bool(result['complete'] and not result['failures'] and
                              len(result.get('octets_array_checks', [])) == 8)
